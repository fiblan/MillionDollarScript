...
wd=over.offsetWidth;ht=over.offsetHeight;txt=backDropSource(wd,ht,zIdx++);txt+='<div style="position: absolute;top: 0;left: 0;width: '+wd+'px;z-index: '+zIdx+';">'+content+'</div>';layerWrite(txt);}
function backDropSource(width,height,Z){if(!olShimObj){olShimObj=document.createElement('iframe');olShimObj.setAttribute("frameborder","0");olShimObj.setAttribute("scrolling","no");olShimObj.setAttribute("src","javascript:false");olShimObj.setAttribute("width",width);olShimObj.setAttribute("height",height);olShimObj.style.zIndex=Z;olShimObj.style.filter="Alpha(Style=0,Opacity=0)";}else{olShimObj.setAttribute("width",width);olShimObj.setAttribute("height",height);olShimObj.style.zIndex=Z;}
return '';}
function setShim(obj){if(!olIe4||olOp||!olIe55)return;var ifrm=obj.getElementsByTagName('IFRAME');if(ifrm.length)obj.replaceChild(olShimObj,ifrm[0]);else obj.insertBefore(olShimObj,obj.firstChild);}
...
registerHook("olCreatePopup",generatePopUp,FAFTER);registerHook("showObject",setShim,FBEFORE);registerHook("hideObject",showSelectBox,FAFTER);olShimObj=null;olHideForm=1;}
